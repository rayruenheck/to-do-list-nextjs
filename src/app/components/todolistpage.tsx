...
export default function ToDoListPage() {
  ...
  const handleLogout = async () => {
    try {
      ...
      const response = await fetch('http://www.raytodolistapi.com/api/logout', {
        method: 'POST',
        headers: {
  ...
  const deleteCompletedTasks = async () => {
    try {
      const response = await fetch(
        "http://www.raytodolistapi.com/api/tasks/delete-completed",
        {
          method: "POST",
        }
      );
  ...
  const addTask = () => {
    ...
    fetch("http://www.raytodolistapi.com/api/tasks", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const getTasks = () => {
    fetch(`http://www.raytodolistapi.com/api/tasks?user_id=${user_id}`)
      .then((response) => {
        if (response.status === 200) {
          return response.json();
        }
